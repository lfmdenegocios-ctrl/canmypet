...
- **Conteúdo em massa de template + cadência agressiva em domínio jovem = perfil de content-farm.** Publicar 493 páginas de comida quase idênticas (~314 palavras únicas, faixa apertada) num domínio de ~1 mês + 3-4 pins/dia → Pinterest bloqueou o domínio inteiro como spam e AdSense recusou. Não é falso positivo puro: o classificador acertou o *padrão* (escalado), errou a *intenção*. Correção (15/jul/2026): **quality gate `strongFood()` no build.js** — só indexa (index + sitemap) páginas com `food.depth`, veredito `danger`, ou `.rich`; o resto recebe `noindex,follow` e sai do sitemap (continua vivo/acessível pelo checker). Sitemap 558→306. Reversível: dar profundidade a um alimento re-indexa sozinho. **Regra daqui pra frente:** domínio novo pede rampa (1 pin/dia, aprofundar antes de escalar); ao sugerir cadência/volume, dizer o RISCO junto, não só a oportunidade. O ativo real são as ferramentas/jogos (não-template, linkáveis), não as 493 páginas.
